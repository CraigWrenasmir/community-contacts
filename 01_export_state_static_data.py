...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export community contacts CSV to docs/data/<state>/ JSON files")
    parser.add_argument("--state", required=True, help="State code, e.g. nsw")
    ...
    for _, row in df.iterrows():
        contacts.append(
            {
                "organisation_name": clean_text(row.get("organisation_name")),
                ...
                "workshop_relevance": clean_text(row.get("workshop_relevance")),
                "lat": float(row["lat"]),
                "lon": float(row["lon"]),
            }
        )
    ...
    (out_dir / "contacts.min.json").write_text(
        json.dumps(contacts, separators=(",", ":"), ensure_ascii=True), encoding="utf-8"
    )
    ...
    print(f"Exported {len(contacts)} rows to docs/data/{state}/")
